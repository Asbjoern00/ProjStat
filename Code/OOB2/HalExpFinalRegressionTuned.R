...
prp_rf_ib <- RF$new(A~., name = "RF Tuned",oob = FALSE, autotune =TRUE,cv = FALSE,list("num.trees" = 500))
mean_rf_ib <- RF$new(Y~., name = "RF Tuned",oob = FALSE, autotune =TRUE,cv = FALSE, hyperparams = list("num.trees" = 500))

prp_rf_oob <-RF$new(A~., name = "RF OOB Tuned",oob = TRUE, autotune =TRUE, cv = FALSE, list("num.trees" = 500))
mean_rf_oob <-RF$new(Y~., name = "RF OOB Tuned",oob = TRUE, autotune =TRUE,cv = FALSE,list("num.trees" = 500))
...
for(i in 1:(length(exps))){
  exps[[i]]$run()
  saveRDS(exps, file = "/home/asr/Desktop/ProjStat/Code/HALExp/HalExpFinalRegressionTuned.rds")
  print(i)
}