  ...
  public = list(  # Public methods and fields
    ...
    fit = function(df){
      ...
      if(self$cross_fit > 0){
        ...
        #Split up datasets for crossfitting. Also setup list to store modified datasets
        split_datasets <- self$split_dataset(df, self$cross_fit)
        rebuild_datasets <- vector(mode = "list", length = self$cross_fit)
        ...
        for(i in seq(self$cross_fit)){
          ...
          #Estimate conditional mean for nobody receiveing treatment
          for_predict[[self$trt_var_name]] <- 0
          cond_mean_ctrl <- self$mean_lrn$predict(for_predict)
          
          #Estimate conditional mean for everyone receiveing treatment
          for_predict[[self$trt_var_name]] <- 1
          cond_mean_trt <- self$mean_lrn$predict(for_predict)
          
          #Assign the actual and counterfactual means
          for_predict[[self$trt_var_name]] <- trtmt
          for_predict$cond_mean_ctrl <- cond_mean_ctrl
          for_predict$cond_mean_trt <- cond_mean_trt
          ...
          #Assign to the rebuild dataset
          rebuild_datasets[[i]] <- for_predict
          
          #Reset Learners for safety (and memory efficacy)
          self$mean_lrn$reset()
          self$prp_lrn$reset()
